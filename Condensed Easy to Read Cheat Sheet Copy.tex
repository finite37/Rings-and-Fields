...
\usepackage{graphicx,blindtext,amsthm,tikz,wrapfig,amssymb,amsmath,xcolor,siunitx,breqn,caption,cancel,esdiff,multicol,multicol,xparse,stmaryrd,hyperref,anyfontsize,tcolorbox} % Required for inserting images
\usepackage[a4paper,landscape,top=0.5cm, bottom=0.3cm, left=0.5cm, right=0.5cm]{geometry}
...
\tcbset{
  statementboxstyle/.style={
    enhanced,breakable,sharp corners,
    colback=yellow!20,colframe=yellow!60!black,
    boxrule=0.22pt,boxsep=0.25pt,
    left=0.4pt,right=0.4pt,top=0.2pt,bottom=0.2pt,
    before skip=0.4pt,after skip=0.4pt
  },
  proofpairboxstyle/.style={
    enhanced,breakable,sharp corners,
    colback=white,colframe=black!80,
    boxrule=0.22pt,boxsep=0.2pt,
    left=0.4pt,right=0.4pt,top=0.2pt,bottom=0.2pt,
...
\newcommand{\defterm}[1]{{\setlength{\fboxsep}{0.45pt}\setlength{\fboxrule}{0.25pt}\fcolorbox{red!70!black}{red!8}{\strut\textbf{#1}}}}

\newenvironment{statementblock}{%
  \begin{tcolorbox}[statementboxstyle]%
}{%
  \end{tcolorbox}%
}
\makeatletter
...
\newcommand{\definetheorem}[3]{%
  \expandafter\newif\csname if#1boxed\endcsname
  \def\thmstyle{#3}\def\plainstyle{plain}%
  \ifx\thmstyle\plainstyle
    \csname #1boxedtrue\endcsname
  \else
    \csname #1boxedfalse\endcsname
  \fi
  \newenvironment{#1}[1]{%
    \refstepcounter{customthm}%           % unique internal label
    \renewcommand{\@currentlabel}{##1}%   % store the custom decimal number
    \par\noindent
    \csname if#1boxed\endcsname\begin{statementblock}\fi
    \csname if#1boxed\endcsname
      \textbf{#2 ##1.}\ \itshape
    ...
    \par
    \csname if#1boxed\endcsname\end{statementblock}\fi
  }%
}
...
\begin{document}
\fontsize{3.6}{4.15}\selectfont
\renewcommand{\footnotesize}{\fontsize{3.35}{3.9}\selectfont}
\renewcommand{\scriptsize}{\fontsize{3.1}{3.6}\selectfont}
    ...
    \raggedright
        ...
        Here, \(r_n\) is the last non-zero remainder and \(\gcd(a,b)=r_n\).
        To prove this relation we show: \(G(r_{i},r_{i+1})=G(r_{i+1},r_{i+2})\), where \(G(a,b)=\{n\in\mathbb{Z}:n|a\;\&\;n|b\}\).

        \textbf{Reverse Euclid:}

        Given a completed algorithm we can obtain \(x,y\in\mathbb{Z}\) s.t. \(xa+yb=\gcd(a,b)\) i.e. taking the start of a Euclidian algorithm:

        \fitdisplay{\(\begin{alignedat}{3}
            a&=q_1b+r_1\quad &&\Rightarrow\quad r_1&&=a-q_1b\\
            b&=q_2r_1+r_2 &&\Rightarrow r_2&&=b-q_2r_1\\
             &&&&&=b-q_2(a-q_1b)\\
             &&&&&=-q_2a+(1+q_1q_2)b\\
            r_1&=q_3r_2+r_3 &&\Rightarrow r_3&&=r_1-q_3r_2\\
             &&&&&=(a-q_1b)\\
             &&&&&\quad -q_3(-q_2a+(1+q_1q_2)b)\\
             &&&&&=(1+q_2q_3)a\\
             &&&&&\quad -(q_1+q_3+q_1q_2q_3)b\\
            &\hspace{5pt}\vdots &&& &\hspace{5pt}\vdots
        \end{alignedat}\)}

        Continue substituting until only \(a,b\) remain:
        {\centering\scriptsize \(r_n=\gcd(a,b)=xa+yb,\quad x,y\in\mathbb{Z}.\)\par}
        \vspace{2pt}
        ...
        \begin{proof}
            We show \(\ker(\vartheta)\) satisfies the ideal conditions. Firstly \(0\in\ker(\vartheta)\). Let \(r,r'\in\ker(\vartheta)\). Then \(\vartheta(r+r')=\vartheta(r)+\vartheta(r')=0+0=0\), so \(r+r'\in\ker(\vartheta)\), and \(\vartheta(-r)=-\vartheta(r)=0\), so \(-r\in\ker(\vartheta)\). Let \(s\in R\). Then \(\vartheta(sr)=\vartheta(s)\vartheta(r)=\vartheta(s)0=0\) and similarly \(\vartheta(rs)=0\). Therefore \(sr,rs\in\ker(\vartheta)\).
        \end{proof}
        ...
        \begin{proof}
            For well-definedness of \(+\) and \(\times\), suppose \(r+I=r'+I\) and \(s+I=s'+I\). Then \(r-r'\in I\) and \(s-s'\in I\). Hence
            \fitdisplay{\((r+s)-(r'+s')=(r-r')+(s-s')\in I,\)}
            so \((r+s)+I=(r'+s')+I\). Also, since \(I\lhd R\),
            \fitdisplay{\(rs-r's'=(r-r')s+r'(s-s')\in I,\)}
            hence \(rs+I=r's'+I\). Thus both operations are independent of the chosen representatives.
        \end{proof}
        ...
        \begin{proof}
            Since \(R\) is commutative, so is \(R/I\), and since \(I\) is proper, \(0+I\ne1+I\). If \(I\) is prime and \((r+I)(s+I)=0+I\), then \(rs\in I\), so \(r\in I\) or \(s\in I\), i.e. \(r+I=0+I\) or \(s+I=0+I\). Conversely, if \(R/I\) is an integral domain and \(rs\in I\), then \((r+I)(s+I)=0+I\), so \(r+I=0+I\) or \(s+I=0+I\), hence \(r\in I\) or \(s\in I\). Thus \(I\) is prime.
        \end{proof}
        ...
        \begin{proof}
            Let \(d=\gcd(f,g)\). Since \(d\mid f\) and \(d\mid g\), \(\langle f,g\rangle\subseteq\langle d\rangle\). The Euclidean algorithm gives \(d=af+bg\) for some \(a,b\in K[X]\), so \(d\in\langle f,g\rangle\), hence \(\langle d\rangle\subseteq\langle f,g\rangle\).
        \end{proof}
        ...
        \begin{definition}{1.6}
            ...
            \(a\) is \defterm{congruent to} \(b\) \defterm{modulo} \(n\) iff \(n|a-b\), written \(a\equiv b\mod n\).Then define the congruence class of \(a\) w.r.t. \(n\) as: \([a]_n=\{b\in\mathbb{Z}:b\equiv a\mod n\}=\{b\in\mathbb{Z}:n\mid(b-a)\}=\{a+kn:k\in\mathbb{Z}\}\)
        ...
        \begin{definition}{1.14}
            A \defterm{ring} is a set \(R\) with two binary operations \(+\) \& \(\times\), defined on \(R\), satisfying the following:
        ...
        \begin{definition}{1.17}
            Let \(R\) be a ring \& \(S\subseteq R\). Then \(S\) is a \defterm{subring} of \(R\) if \(S\) is a ring w.r.t. the same addition \& multiplication as \(R\) \& contains the multiplicative identity of \(R\), \(1_R\).
        \end{definition}

        \begin{definition}{1.21}
            let \(R\) be a ring. The \defterm{ring of polynomials} \(R[X]=\{\sum_{i\ge0}^na_iX^i\text{ a finite sum}:a_i\in R\}\) with addition \& multiplication defined as:
            ...
            \(0=\sum_{i\ge0}0X^i\) \& \(1=1X^0+\sum_{i\ge1}0X^i\). Then the \defterm{degree} of a polynomial is the greatest \(i\in\mathbb{N}\cup\{0\}\) s.t. \(a_i\ne0\) or if no such \(i\) exist \(\deg(0)=-\infty\)
        ...
        \begin{definition}{1.24}
            If \(R_1\) \& \(R_2\) are rings then the \defterm{Cartesian product} \(R_1\times R_2\) with the \(+\) \& \(\times\) defined element wise is a ring.
        \end{definition}
        ...
        \begin{definition}{2.1}
            ...
            The \defterm{Characteristic}, \(\text{char}(R)\), of a ring \(R\) is the least posititve integer \(n\) s.t. \(n\cdot 1=0\), or if no exist \(\text{char}(R)=0\).
        ...
        \begin{definition}{2.3}
            A non-zero element \(r\in R\) is a \defterm{zero-divisor} if \(\exists s\in R\) a non-zero element s.t. \(rs=0\) or \(sr=0\).
        ...
        \begin{definition}{2.9}
            A \defterm{division ring} is a ring where every non-zero element has a right \& left inverse, i.e.: \(\forall r\in R\setminus\{0\},\exists s,t\in R\text{ s.t. }rs=1=tr\) with \(s\),\(t\) being the right \& left inverses, respectively.

            {\footnotesize Note: this immediately collapse, due to Le\textsuperscript{m} 2.11, to just there exists an inverse as \(s=t\).}
            
            An \defterm{invertible} element \(r\) is called a \defterm{unit}.
        \end{definition}

       
        \begin{definition}{2.16}
            An element \(r\) of a ring \(R\) is \defterm{nilpotent} if \(\exists n\in\mathbb{Z}^+\) s.t. \(r^n=0\), and the least such \(n\) is the \defterm{index of nilpotence} of \(r\).

            An element \(r\in R\) is \defterm{idempotent} if \(r^2=r\).

            {\footnotesize Note: In any ring \(R\), \(0_R\) is nilpotent with index 1 and \(0_R\) and \(1_R\) are idempotent.}
        \end{definition}

        \begin{definition}{3.1}
            If \(R\) \& \(S\) are rings then an \defterm{isomorphism} from \(R\) to \(S\) is a bijection \(\phi:R\to S\) s.t. \(\forall r,r'\in R\):

            \fitdisplay{\(\begin{aligned}\phi(r+r')&=\phi(r)+\phi(r'),\\ \phi(rr')&=\phi(r)\phi(r')\end{aligned}\)}

            \(\exists\phi:R\to S\) an isomorphism iff \(R\) \& \(S\) are \defterm{isomorphic}, written \(R\simeq S\).
        \end{definition}


        \begin{definition}{3.6}
            If \(R\) \& \(S\) are rings then a \defterm{homomorphism} from \(R\) to \(S\) is a map \(\vartheta:R\to S\) s.t. \(\forall r,r'\in R\):
        ...
        \begin{definition}{3.9}
            An \defterm{embedding} or \defterm{monomorphism} is an injective homomorphism.
        ...
        \begin{definition}{3.12}
            If \(\vartheta :R\to S\) is a ring homomorphism, then the \defterm{kernel} of \(\vartheta\) is defiend as \(\text{ker}(\vartheta)=\{r\in R:\vartheta(r)=0_S\}\).
        \end{definition}

        \begin{definition}{3.16}
            An \defterm{automorphism} of a ring is an isomorphism from and to itself.
        \end{definition}


        \begin{definition}{3.18}
            An \defterm{ideal} of a ring \(R\) is a subset \(I\subseteq R\) s.t.: \(0\in I\); \(a+b\in I\), \(\forall a,b\in I\); \& \(ar,ra\in I\), \(\forall a\in I\), \(\forall r\in R\). We write \(I\lhd R\) to mean \(I\) is an ideal of \(R\).
        \end{definition}

        \begin{definition}{3.19}
            If \(a\in R\) then \(\{r_1as_1+\cdots+r_nas_n:n\ge1,\;r_i,s_i\in R\}\) is an ideal containing \(a\) and is the smallest ideal of \(R\) containing \(a\). It is called the \defterm{principal ideal generated by} \(a\), denoted \(\langle a\rangle\). If \(R\) is commutative then \(\langle a\rangle=\{ar:r\in R\}\). A \defterm{principal ideal} is one which can be generated by a single element.
        \end{definition}

        \begin{definition}{3.21}
            A \defterm{right ideal} is defined as for a general ideal but with the third condition replaced by the weaker condition: \(a\in I\) and \(r\in R\) implies \(ar\in I\). A \defterm{left ideal} is defined similarly. If \(a\in R\), the \defterm{principal right ideal generated by} \(a\) is \(\{ar:r\in R\}\), denoted \(aR\).
        \end{definition}


        \begin{definition}{3.22}
            Let \(A\subseteq R\) and \(R\) be a commutative ring, then \(\{a_1r_1+\cdots+a_nr_n:n\ge1,a_i\in A,r_i\in R\}\) is the \defterm{ideal generated by} \(A\).
        \end{definition}


        \begin{definition}{3.26}
            We can also define a \defterm{product of ideals}: if \(I,J\lhd R\), then their product is the ideal generated by \(\{ab:a\in I,b\in J\}\). This set will not in general be closed under addition, hence we say ``generated by''. Powers of an ideal are defined inductively by \(I^{n+1}=I^nI\).
        \end{definition}
        ...
        \begin{definition}{4.1}
            The ring \(R/I\) is called the \defterm{factor ring} (or \defterm{quotient ring}) of \(R\) by \(I\).
        \end{definition}
        ...
        \begin{definition}{4.4}
            The map \(\pi:R\to R/I\), \(\pi(r)=r+I\), is called the \defterm{canonical projection}, or \defterm{canonical surjection}.
        \end{definition}
        ...
        \begin{definition}{4.12}
            An ideal \(I\) of a ring \(R\) is \defterm{maximal} if it is proper and for any ideal \(J\) with \(I\le J\le R\), either \(J=I\) or \(J=R\).
        \end{definition}

        \begin{definition}{4.14}
            A proper ideal \(I\) of a commutative ring \(R\) is \defterm{prime} if whenever \(r,s\in R\) and \(rs\in I\), then \(r\in I\) or \(s\in I\).
        \end{definition}

        \begin{definition}{5.3}
            An element \(a\in K\) is a \defterm{root} (or \defterm{zero}) of \(f\in K[X]\) iff \(f(a)=0\).
        \end{definition}

        \begin{definition}{5.5}
            The \defterm{greatest common divisor} of polynomials \(f,g\) is a polynomial \(d\) such that \(d\mid f\) and \(d\mid g\), and if \(h\mid f\) and \(h\mid g\), then \(h\mid d\). This determines \(d\) up to a non-zero scalar; for uniqueness take \(d\) \defterm{monic}.
        \end{definition}

        \defterm{Division Algorithm for Polynomials}. For \(f,g\in K[X]\), \(g\ne0\), repeatedly apply Th\textsuperscript{m} 5.1:
        \fitdisplay{\(\begin{aligned}
        ...
        \defterm{Factorisation}. Here \(R\) is commutative.

        \begin{definition}{5.9}
            (i) \(r\in R\) is \defterm{irreducible} if \(r\) is not invertible and whenever \(r=st\), either \(s\) or \(t\) is invertible. (ii) \(r,s\in R\) are \defterm{associated} if \(s=ur\) for some invertible \(u\in R\).
        \end{definition}

        \begin{definition}{5.11}
            A commutative domain \(R\) is a \defterm{Unique Factorisation Domain (UFD)} if every non-zero, non-invertible element factors uniquely into irreducibles, up to rearrangement and associated factors.
        \end{definition}
        \begin{definition}{5.14}
            A \defterm{Principal Ideal Domain (PID)} is a commutative domain in which every ideal is principal.
        \end{definition}
        ...
        \begin{samepage}
        \vspace{2pt}
        {\centering\fontsize{6}{6.8}\selectfont\textbf{Examples}\par}
        \vspace{2pt}
        ...
        \begin{example}
            \textbf{Examples 3.20.}
            (i) In every ring \(\langle0\rangle=\{0\}\) is the smallest ideal, the \defterm{trivial ideal}.
            (ii) In every ring \(\langle1\rangle=R\) is an ideal and every other ideal \(\ne R\) is a \defterm{proper ideal}.
        ...
        \begin{example}
            \textbf{4.11.} Since \(\langle6\rangle\le\langle2\rangle\), Th\textsuperscript{m} 4.10 gives
            \fitdisplay{\((\mathbb{Z}/\langle6\rangle)/(\langle2\rangle/\langle6\rangle)\simeq\mathbb{Z}/\langle2\rangle\).}
        \end{example}
        ...
        \begin{example}
            \textbf{Examples 5.10.}
            (i) In \(\mathbb{Z}\), irreducibles are \(\pm p\) for \(p\) prime; \(r,s\) are associated iff \(r=\pm s\). (ii) In \(K[X]\), \(K\) a field, irreducibles are non-constant polynomials with no factor \(g\) satisfying \(1\le\deg(g)<\deg(f)\).
        \end{example}
        ...
        \begin{center}
            ...
            \begin{tikzpicture}
                \path[use as bounding box] (-6.08,-3.58) rectangle (3.65,4.4);
                \clip (-6.08,-3.58) rectangle (3.65,4.4);
                \draw [rotate around={23.064727066836003:(-1.15350905838475,0.4052823669666326)},line width=1.2pt,dotted] (-1.15350905838475,0.4052823669666326) ellipse (4.955070892823739cm and 3.637174719019717cm);
                \draw [line width=2pt] (-5,1)-- (-5,-2);
                \draw [line width=2pt] (-5,-2)-- (-4,-2);
                \draw [line width=2pt] (-4,-2)-- (-4,-3);
                \draw [line width=2pt] (-4,-3)-- (0,-3);
                \draw [line width=2pt] (0,-3)-- (0,-2);
                \draw [line width=2pt] (0,-2)-- (1,-2);
                \draw [line width=2pt] (1,-2)-- (1,-1);
                ...
                \draw [line width=2pt] (3,3)-- (2,3);
                \draw [line width=2pt] (2,3)-- (1,3);
                \draw [line width=2pt] (1,3)-- (1,4);
                \draw [line width=2pt] (1,4)-- (-1,4);
                \draw [line width=2pt] (-1,4)-- (-1,3);
                \draw [line width=2pt] (-1,3)-- (-3,3);
                \draw [line width=2pt] (-3,3)-- (-3,2);
                \draw [line width=2pt] (-3,2)-- (-4,2);
                \draw [line width=2pt] (-4,2)-- (-4,1);
                ...
                \draw [line width=2pt] (-4,1)-- (3,1);
                \draw [line width=2pt] (-5,0)-- (2,0);
                \draw [line width=2pt] (-5,-1)-- (1,-1);
                \draw [line width=2pt] (-4,-2)-- (0,-2);
                \draw [line width=2pt] (-4,-2.5)-- (-4.5,-2.5);
                \draw [line width=2pt] (-4.5,-2.5)-- (-5,-2.5);
                \draw [line width=2pt] (-5,-2.5)-- (-5,-2);
                \draw [line width=2pt] (-5,-2)-- (-5,-1.5);
                \draw [line width=2pt] (-5,-1.5)-- (-5.5,-1.5);
                \draw [line width=2pt] (-5.5,-1.5)-- (-5.5,1);
                \draw [line width=2pt] (-5,1)-- (-5.5,1);
                \draw [line width=2pt] (-5.5,0.5)-- (-5,0.5);
                \draw [line width=2pt] (-5.5,0)-- (-5,0);
                \draw [line width=2pt] (-5.5,-0.5)-- (-5,-0.5);
                \draw [line width=2pt] (-5.5,-1)-- (-5,-1);
                \draw [line width=2pt] (-5,1)-- (-5,1.5);
                \draw [line width=2pt] (-5,1.5)-- (-4,1.5);
                \draw [line width=2pt] (-4.5,1.5)-- (-4.5,1);
                \draw [line width=2pt] (-4.5,1.5)-- (-4.5,2);
                \draw [line width=2pt] (-4.5,2)-- (-4,2);
                \draw [line width=2pt] (-4,2)-- (-4,2.5);
                \draw [line width=2pt] (-4,2.5)-- (-3.5,2.5);
                \draw [line width=2pt] (-3.5,2.5)-- (-3.5,2);
                \draw [line width=2pt] (-3.5,2.5)-- (-3.5,3);
                \draw [line width=2pt] (-3.5,3)-- (-3,3);
                \draw [line width=2pt] (-3.5,2.5)-- (-3,2.5);
                \draw [line width=2pt] (-3,3)-- (-3,3.5);
                \draw [line width=2pt] (-1.5,4)-- (-1,4);
                \draw [line width=2pt] (1,3.5)-- (1.5,3.5);
                \draw [line width=2pt] (1.5,3.5)-- (1.5,3);
                \draw [line width=2pt] (1.5,3.5)-- (2,3.5);
                \draw [line width=2pt] (2,3.5)-- (2,3);
                \draw [line width=2pt] (3,2)-- (3.5,2);
                \draw [line width=2pt] (3.5,2)-- (3.5,1);
                \draw [line width=2pt] (3.5,1)-- (3.5,0.5);
                \draw [line width=2pt] (3,0.5)-- (3.5,0.5);
                \draw [line width=2pt] (3,1)-- (3.5,1);
                \draw [line width=2pt] (3,1.5)-- (3.5,1.5);
                \draw [line width=2pt] (3,0)-- (3,-0.5);
                \draw [line width=2pt] (3,-0.5)-- (2,-0.5);
                \draw [line width=2pt] (2.5,0)-- (2.5,-1);
                \draw [line width=2pt] (2.5,-1)-- (2,-1);
                \draw [line width=2pt] (1,-2)-- (1.5,-2);
                \draw [line width=2pt] (1.5,-2)-- (1.5,-1);
                \draw [line width=2pt] (1,-1.5)-- (2,-1.5);
                \draw [line width=2pt] (2,-1.5)-- (2,-1);
                \draw [line width=2pt] (0.5,-2)-- (0.5,-2.5);
                \draw [line width=2pt] (0.5,-2.5)-- (0,-2.5);
                \draw [line width=2pt] (0.5,-2.5)-- (1,-2.5);
                \draw [line width=2pt] (1,-2.5)-- (1,-2);
                \draw [line width=2pt] (-4.5,-2.5)-- (-4.5,-2);
                \draw [line width=2pt] (-3,3.5)-- (-2,3.5);
                \draw [line width=2pt] (-2,3)-- (-2,3.5);
                \draw [line width=2pt] (-2,3.5)-- (-1.5,3.5);
                \draw [line width=2pt] (-1.5,3.5)-- (-1.5,4);
                \draw [line width=2pt] (-1.5,3.5)-- (-1.5,3);
                \draw [line width=2pt] (-1.5,3.5)-- (-1,3.5);
                \draw [line width=2pt] (-2.5,3.5)-- (-2.5,3);
                \draw [line width=2pt] (-4.5,-2.5)-- (-4.5,-2.75);
                \draw [line width=2pt] (-4.5,-2.75)-- (-4.25,-2.75);
                \draw [line width=2pt] (-4.25,-2.75)-- (-4.25,-2.5);
                \draw [line width=2pt] (-4.25,-2.75)-- (-4.25,-3);
                \draw [line width=2pt] (-4.25,-3)-- (-4,-3);
                \draw [line width=2pt] (-4.25,-2.75)-- (-4,-2.75);
                \draw [line width=2pt] (-5,-2.25)-- (-5.25,-2.25);
                \draw [line width=2pt] (-5.25,-2.25)-- (-5.25,-2);
                \draw [line width=2pt] (-5.25,-2)-- (-5,-2);
                \draw [line width=2pt] (-5.25,-2)-- (-5.25,-1.75);
                \draw [line width=2pt] (-5.25,-1.75)-- (-5,-1.75);
                \draw [line width=2pt] (-5.25,-1.75)-- (-5.5,-1.75);
                \draw [line width=2pt] (-5.5,-1.75)-- (-5.5,-1.5);
                \draw [line width=2pt] (-5.25,-1.75)-- (-5.25,-1.5);
                \draw [line width=2pt] (-5.5,-1.25)-- (-5.75,-1.25);
                \draw [line width=2pt] (-5.75,-1)-- (-5.75,-1.25);
                \draw [line width=2pt] (-5.75,-1)-- (-5.5,-1);
                \draw [line width=2pt] (-5.75,-1)-- (-5.75,-0.75);
                \draw [line width=2pt] (-5.75,-0.75)-- (-5.5,-0.75);
                \draw [line width=2pt] (-5.75,-0.75)-- (-5.75,-0.5);
                \draw [line width=2pt] (-5.75,-0.5)-- (-5.5,-0.5);
                \draw [line width=2pt] (-5.75,-0.5)-- (-5.75,-0.25);
                \draw [line width=2pt] (-5.75,-0.25)-- (-5.5,-0.25);
                \draw [line width=2pt] (-5.75,-0.25)-- (-5.75,0);
                \draw [line width=2pt] (-5.75,0)-- (-5.5,0);
                \draw [line width=2pt] (-5.75,0)-- (-5.75,0.25);
                \draw [line width=2pt] (-5.75,0.25)-- (-5.5,0.25);
                \draw [line width=2pt] (-5.75,0.25)-- (-5.75,0.5);
                \draw [line width=2pt] (-5.75,0.5)-- (-5.5,0.5);
                \draw [line width=2pt] (-5.25,1)-- (-5.25,1.25);
                \draw [line width=2pt] (-5.25,1.25)-- (-5,1.25);
                \draw [line width=2pt] (-5.25,1.25)-- (-5.25,1.5);
                \draw [line width=2pt] (-5.25,1.5)-- (-5,1.5);
                \draw [line width=2pt] (-5,1.5)-- (-5,1.75);
                \draw [line width=2pt] (-5,1.75)-- (-4.5,1.75);
                \draw [line width=2pt] (-4.75,1.5)-- (-4.75,2);
                \draw [line width=2pt] (-4.75,2)-- (-4.5,2);
                \draw [line width=2pt] (-4.5,2)-- (-4.5,2.25);
                \draw [line width=2pt] (-4.5,2.25)-- (-4,2.25);
                \draw [line width=2pt] (-4.25,2)-- (-4.25,2.5);
                \draw [line width=2pt] (-4.25,2.5)-- (-4,2.5);
                \draw [line width=2pt] (-4,2.5)-- (-4,2.75);
                \draw [line width=2pt] (-4,2.75)-- (-3.5,2.75);
                \draw [line width=2pt] (-3.75,2.5)-- (-3.75,3);
                \draw [line width=2pt] (-3.75,3)-- (-3.5,3);
                \draw [line width=2pt] (-3.5,3)-- (-3.5,3.25);
                \draw [line width=2pt] (-3.5,3.25)-- (-3,3.25);
                \draw [line width=2pt] (-3.25,3)-- (-3.25,3.25);
                \draw [line width=2pt] (-2.5,3.5)-- (-2.5,3.75);
                \draw [line width=2pt] (-2.5,3.75)-- (-1.5,3.75);
                \draw [line width=2pt] (-2.25,3.5)-- (-2.25,3.75);
                \draw [line width=2pt] (-2,3.5)-- (-2,3.75);
                \draw [line width=2pt] (-1.75,3.5)-- (-1.75,3.75);
                \draw [line width=2pt] (-1.75,3.75)-- (-1.75,4);
                \draw [line width=2pt] (-1.75,4)-- (-1.5,4);
                \draw [line width=2pt] (-1,4)-- (-0.75,4);
                \draw [line width=2pt] (-0.75,4)-- (-0.5,4);
                \draw [line width=2pt] (-0.5,4)-- (-0.5,4.25);
                \draw [line width=2pt] (-0.5,4.25)-- (0.25,4.25);
                \draw [line width=2pt] (0,4)-- (0,4.25);
                \draw [line width=2pt] (-0.25,4)-- (-0.25,4.25);
                \draw [line width=2pt] (0.25,4.25)-- (0.25,4);
                \draw [line width=2pt] (1,4)-- (1.5,4);
                \draw [line width=2pt] (1.5,4)-- (1.5,3.5);
                \draw [line width=2pt] (1.5,3.75)-- (1.75,3.75);
                \draw [line width=2pt] (1.75,3.75)-- (1.75,3.5);
                \draw [line width=2pt] (1.75,3.75)-- (2,3.75);
                \draw [line width=2pt] (2,3.75)-- (2,3.5);
                \draw [line width=2pt] (2,3.5)-- (2.25,3.5);
                \draw [line width=2pt] (2.25,3.5)-- (2.25,3);
                \draw [line width=2pt] (2,3.25)-- (2.5,3.25);
                \draw [line width=2pt] (2.5,3.25)-- (2.5,3);
                \draw [line width=2pt] (2.5,3.25)-- (2.75,3.25);
                \draw [line width=2pt] (2.75,3.25)-- (2.75,3);
                \draw [line width=2pt] (3,3)-- (3,2.75);
                \draw [line width=2pt] (3,2.75)-- (3,2.5);
                \draw [line width=2pt] (3,2.5)-- (3.25,2.5);
                \draw [line width=2pt] (3.25,2.5)-- (3.25,2);
                \draw [line width=2pt] (3,2.25)-- (3.25,2.25);
                \draw [line width=2pt] (3.25,0.5)-- (3.25,0);
                \draw [line width=2pt] (3,0)-- (3.25,0);
                \draw [line width=2pt] (3,0.25)-- (3.25,0.25);
                \draw [line width=2pt] (3,-0.25)-- (3.25,-0.25);
                \draw [line width=2pt] (3.25,-0.25)-- (3.25,0);
                \draw [line width=2pt] (2.5,-1)-- (2.75,-1);
                \draw [line width=2pt] (2.75,-0.5)-- (2.75,-1);
                \draw [line width=2pt] (2.5,-0.75)-- (2.75,-0.75);
                \draw [line width=2pt] (2,-1.5)-- (2.25,-1.5);
                \draw [line width=2pt] (2.25,-1.5)-- (2.25,-1);
                \draw [line width=2pt] (2,-1.25)-- (2.5,-1.25);
                \draw [line width=2pt] (2.5,-1.25)-- (2.5,-1);
                \draw [line width=2pt] (1.5,-2)-- (1.75,-2);
                \draw [line width=2pt] (1.75,-2)-- (1.75,-1.5);
                \draw [line width=2pt] (1.5,-1.75)-- (2,-1.75);
                \draw [line width=2pt] (2,-1.75)-- (2,-1.5);
                \draw [line width=2pt] (1.5,-2)-- (1.5,-2.25);
                \draw [line width=2pt] (1.5,-2.25)-- (1,-2.25);
                \draw [line width=2pt] (1.25,-2.25)-- (1.25,-2);
                \draw [line width=2pt] (0.5,-2.5)-- (0.5,-2.75);
                \draw [line width=2pt] (0.5,-2.75)-- (0,-2.75);
                \draw [line width=2pt] (0.25,-2.5)-- (0.25,-2.75);
                \draw [line width=2pt] (-4,-3)-- (-3.75,-3);
                \draw [line width=2pt] (-3.75,-3)-- (-3.5,-3);
                \draw [line width=2pt] (-3.5,-3)-- (-3.25,-3);
                \draw [line width=2pt] (-3.25,-3)-- (-3,-3);
                \draw [line width=2pt] (-3.5,-3)-- (-3.5,-3.25);
                \draw [line width=2pt] (-3.5,-3.25)-- (-3,-3.25);
                \draw [line width=2pt] (-3,-3.25)-- (-3,-3);
                \draw [line width=2pt] (-3.25,-3)-- (-3.25,-3.25);
                \draw [line width=2pt] (-3,-3.25)-- (-1,-3.25);
                \draw [line width=2pt] (-1,-3.25)-- (-1,-3);
                \draw [line width=2pt] (-2.75,-3.25)-- (-2.75,-3);
                \draw [line width=2pt] (-2.5,-3)-- (-2.5,-3.25);
                \draw [line width=2pt] (-2.25,-3)-- (-2.25,-3.25);
                \draw [line width=2pt] (-2,-3)-- (-2,-3.25);
                \draw [line width=2pt] (-1.75,-3)-- (-1.75,-3.25);
                \draw [line width=2pt] (-1.5,-3)-- (-1.5,-3.25);
                \draw [line width=2pt] (-1.25,-3)-- (-1.25,-3.25);
                \draw [line width=2pt] (-1,-3.25)-- (-0.75,-3.25);
                \draw [line width=2pt] (-0.75,-3.25)-- (-0.75,-3);
            \end{tikzpicture}%
            }
            \vspace{-10pt}
        \end{center}
